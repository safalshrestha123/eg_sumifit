        ...
        <Dialog key={`${item.alt}-${index}`}>
          <DialogTrigger asChild>
            <button className={cn("group relative overflow-hidden rounded-3xl bg-gray-100 text-left dark:bg-gray-900", item.className)} aria-label={`Open ${item.alt}`}>
              <Image src={item.src} alt={item.alt} fill sizes="(min-width: 768px) 33vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" />
          ...
          <DialogContent>
            ...
            <div className="relative h-[80vh] w-full"><Image src={item.src} alt={item.alt} fill sizes="90vw" className="object-contain" /></div>
          </DialogContent>
        </Dialog>
      ))}
    </div>
